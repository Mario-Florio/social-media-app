...
async function update(req, res, next) {
    try {    
        ...
        if (!post) {
            return res.status(400).json({ message: "Post does not exist", success: false });
        }
    
        const userId = post.user._id.toString();
        const { authData } = verifyTokenResBody;
        if (authData.user._id !== userId) {
            return res.status(404).json({ message: "You are not authorized to delete this post", success: false });
        }
