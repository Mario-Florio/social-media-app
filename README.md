...
**Disclaimer: The demo version does not consist of any features linked to a database (e.g. saving user data, uploading images, etc.) and is simply for viewing the contents of the frontend. Users of demo version will be able to post, view other (mock) users profile, comment on others posts, and edit a profile. Any changes made to the data during the demo run will be reset upon refreshing the page (including any accounts made). For access to full featured version, see the [setup guide](#setup) to run application on your local machine.**
